...
mapper はユーザー固有の設定であるため、プラグインとしては配布しない。ただし特定アバター向けに作られた mapper を、ワークスペース内の依存アダプターごと配布用プロファイルパッケージに同梱して配布する経路は別にある（プラグイン由来のアダプターは同梱せず依存プラグインとしてマニフェストに記録。プラグインとは別機構 → [`config/05-profile.md`](config/05-profile.md)「配布用プロファイル」）。
