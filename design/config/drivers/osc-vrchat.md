...
VRChat の OSC アバターパラメーター専用ドライバー。`osc` とは独立した別ドライバーとして扱う。
...
| フィールド | 必須 | 説明 |
|---|---|---|
| `target` | ✅ | `/avatar/parameters/<ParameterName>` 形式。`{note}` キャプチャ変数を含められる |
...
`osc` ドライバーと同一の構文。`address` に `/avatar/parameters/<ParameterName>` 形式を使う。
