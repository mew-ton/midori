...
| `knob` | `<component_id>.<value_name>` | `volume_knob.value` |
| `switch` | `<component_id>.<value_name>` | `upper_sustain.pressed` |
| `pulser` | `<component_id>.<value_name>` | `rhythm_start.triggered` |

`{note}` はイベント到着時に note 番号（0–127）へ展開される。keyboard 以外には不要。

`*`（全要素 gather）は変換グラフ専用。binding では使用不可。
...
| 記法 | YAML 型 | 意味 | 例 |
|---|---|---|---|
| 数値のみ | integer | 完全一致 | `when: 64` |
| `"< N"` | string | N 未満 | `when: "< 64"` |
| `"<= N"` | string | N 以下 | `when: "<= 63"` |
| `"> N"` | string | N より大きい | `when: "> 63"` |
| `">= N"` | string | N 以上 | `when: ">= 64"` |
| `"A~B"` | string | A 以上 B 以下（inclusive） | `when: "0~63"` |

完全一致は YAML ネイティブ整数で記述する。演算子・範囲を含む場合は文字列で記述する。
...
binding:
  input:
    driver: midi
    mappings:
      # --- note フィールドあり: {note} は自動解決 ---
      - from:
          channel: 1
          type: noteOn                  # note フィールドあり → {note} 自動解決
        to:
      ...
      - from:
          channel: 1
          type: pitchBend               # from は受信信号の記述のみ。note に関する情報は持たない
        to:
          target: upper.60.lateral      # note 番号をリテラルで直接指定
          set: value                    # -8192–8191 → -1~1 に自動正規化

      # --- keyboard 以外: {note} 不要、from.note 指定も不要 ---
      - from:
          channel: 1
          type: controlChange
...
## MIDI ドライバーで受け付けるイベント type

| type | 意味 |
|---|---|
| `noteOn` | ノートオン |
| `noteOff` | ノートオフ |
| `controlChange` | コントロールチェンジ（`controller` フィールドで番号指定） |
| `pitchBend` | ピッチベンド |
| `polyAftertouch` | ポリアフタータッチ |
| `channelAftertouch` | チャンネルアフタータッチ |
