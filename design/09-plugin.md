...
mapper はユーザー固有の設定であるため、プラグインとしては配布しない。ただし特定アバター向けに作られた mapper を、その依存アダプターごと配布用プロファイルパッケージに同梱して配布する経路は別にある（プラグインとは別機構 → [`config/05-profile.md`](config/05-profile.md)「配布用プロファイル」）。
