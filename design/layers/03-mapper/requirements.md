# Layer 3 — 変換グラフ 要件

## 責務

ComponentState を Signal に変換する。入力値の条件フィルタリングと値変換を担う。

## インターフェース

```
入力: ComponentState（Layer 2 出力）
出力: Signal
```

---

## GUI モデル：ノードグラフ

変換グラフ の編集は**ノードをつないで構成するグラフ形式**の GUI を想定する。

```
┌──────────────┐        ┌──────────────┐        ┌──────────────┐
│ 入力ブロック  │──────▶│  計算ノード   │──────▶│ 出力ブロック  │
│（デバイスごと）│        │（変換・加工）  │        │（デバイスごと）│
└──────────────┘        └──────────────┘        └──────────────┘
```

- **入力ブロック**：入力デバイス1つにつき1ブロック。`device_id` をタイトルとし、そのアダプターの definition に基づいてポートを自動生成。複数 inputs がある場合は縦に並ぶ。タイトルクリックで折りたたみ可能（折りたたみ時はタイトルのみ表示）
- **出力ブロック**：出力デバイス1つにつき1ブロック。`device_id` をタイトルとし、Signal 指定子でポートを命名。複数 outputs がある場合は縦に並ぶ。同様に折りたたみ可能
- **計算ノード**：Input と Output の間に任意に挟む。複数つなげてチェーンにできる

計算ノードなしで Input → Output を直結することもできる。

### GUI の実装モデル

ノードグラフ UI は責務を明確に2層に分ける。

| 層 | 技術 | 責務 |
|---|---|---|
| ノード本体 | Svelte（Astro Island） | ノードの内部状態・パラメーター編集・ポートのリアクティブな値管理 |
| 接続線 | Web Component + 動的 SVG | ノード間のベジエ曲線描画。ノード位置を自律監視して自動再描画 |

接続線の Web Component（`<node-wire>`）は `from` / `to` 属性に接続元・先のポート要素を受け取り、`ResizeObserver` でノード位置の変化を監視して SVG ベジエを自動再描画する。Svelte 側はノードを動かすだけでよく、線の描画を関知しない。

---

## ポート型システム

各ポートは型を持つ。**型が一致するポートにしか接続できない**。型不一致の接続はバリデーションエラーとする。

スカラー型（`bool` / `float` / `int` / `pulse`）の定義 → [config/syntax/02-value-types.md](../../config/syntax/02-value-types.md)

配列型（`bool[]` / `float[]` / `int[]`）は `*` ワイルドカード接続（gather）で生成される。スカラーポートに接続するには `flatten` ノードで展開する必要がある。

### 型変換ルール

暗黙の型変換は行わない。異なる型のポートをつなぐには変換ノードを明示的に挟む。

型変換に使えるノード一覧 → [config/mapper-nodes/](../../config/mapper-nodes/)

ノード一覧・接続記法（`{note}` / `*`）・設定 YAML 仕様 → [config/03-mapper.md](../../config/03-mapper.md)

---

## 要件

| # | 要件 | 補足 |
|---|---|---|
| 1 | グラフのポートが参照する device_id はプロファイルの `inputs[].id` / `outputs[].id` と一致すること | ランタイムがプロファイルをもとにアダプターを解決する |
| 2 | 入力ブロックのポートは入力アダプターの definition から自動生成されること | 手動で列挙しない。デバイスごとに1ブロック |
| 3 | 計算ノードを Input と Output の間に任意に挟めること | 0個でも複数でも可 |
| 4 | 計算ノードを直列につなげてチェーンにできること | |
| 5 | ノードは複数の名前付き入力ポートを持てること | 単入力の `in` / 複数入力の `tempo`, `beat` 等 |
| 6 | ノードは複数の名前付き出力ポートを持てること | 単出力の `out` / 複数出力の `beat_0`, `beat_1` 等 |
| 7 | 入力ポートへの接続は動的（他ノードの出力）と静的（literal 定数）の両方を取れること | ノード全体の定数は `params`、ポート単位の静的値は connections 内の `value:` で記述 |
| 8 | ステートを持つノードが存在できること | metronome の拍カウント等 |
| 9 | keyboard 等の配列型 component は `{note}` でテンプレート展開できること | 接続1本の定義が全キーに適用される |
| 10 | 型が一致しないポート間の接続はエラーとすること | 起動時バリデーション。暗黙変換なし |
| 11 | 入力ブロックの存在しないポートへの接続はエラーとすること | 起動時バリデーション |
| 12 | 個人ファイルとしての受け渡し、または特定アバター向けに作られたものを配布用プロファイルパッケージへ同梱しての配布を前提とすること（プライベート共有） | アバター・演奏スタイルに依存し、汎用の公開部品にはしない |

---

## 現時点で対応しないこと（将来拡張ポイント）

- 外部データ参照

## 設定仕様

→ [config/03-mapper.md](../../config/03-mapper.md)
