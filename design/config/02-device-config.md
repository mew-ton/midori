...
| 形式 | 例 | 説明 | 正規化 |
|---|---|---|---|
| スカラーリテラル | `set: 1` | リテラル値をそのまま書き込む | なし（値をそのまま代入） |
...
| `{ expr: 式 }` | `set: { expr: "(hi << 2) \| (lo >> 5)" }` | 式を評価した整数結果を target 型に変換して書き込む（`float` は正規化なし・直接代入、`bool` は 0→false / それ以外→true）。式の中で値域の計算まで完結させること。詳細は [式言語仕様](./expr.md) を参照 | なし |
...
**前提条件**: `mirror` は `binding.input` が有効なドライバーでのみ使用可能。ドライバーが input をサポートしていない（例: `direction: output` のデバイスで使われる OSC ドライバーなど）場合は validation error。

逆写像が導出できないケース（いずれもエラー）：

- ドライバーが input をサポートしていない
- 同一 `to.target` に複数の異なる入力経路がある（例: CC と SysEx の両方が同じ target を更新）
- `setMap.map` が非全単射（複数の入力値が同じ出力値に写る）
...
- `set: リテラル` で同じ `to.target` を持つエントリがペアになっている（例: noteOn `set: 1` / noteOff `set: 0`）
