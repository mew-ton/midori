...
ドライバーは CLI サブコマンド `list` を提供する（`draft-driver-sdk.md` 参照）。GUI がウィジェット表示時に `<driver> list` を実行し、返ってきた JSON でドロップダウンを生成する。

```
GUI がウィジェットを描画
  → <driver> list を実行（一時プロセス）
  → JSON で選択肢を受け取る
  → ドロップダウンに表示
ユーザーが選択
  → 選択値をプロファイルに保存
  → Bridge 起動時に <driver> start --device "..." で渡す
```

BLE 等のスキャンが必要なデバイスも同じ仕組みで扱える。`<driver> list` が「スキャンして返す」実装をするだけで、GUI 側は変わらない。

### 標準ウィジェット型

| type | 表示 | リスト取得 | 用途 |
|---|---|---|---|
| `device-select` | ドロップダウン | `<driver> list` を実行 | MIDI・BLE 等 |
| `host-port` | ホスト名 + ポート番号の入力欄ペア | — | OSC |
| `port` | ポート番号のみ | — | OSC 受信専用ポート等 |
| `file` | ファイルパス選択ダイアログ | — | アバター JSON 等 |
| `text` | テキスト入力 | — | 汎用 |

`scan` は `device-select` に統合。ドライバーの `list` 実装がスキャンを含めるかどうかはドライバー側の責務であり、GUI は区別しない。
