...
3. Driver / Bridge の同期は **ring の `write_index` / `read_index` と独立した
   `side_read_index` AtomicU64 ひとつ** で表現する
...
ページサイズ（4 KiB）アラインで開始する（mmap セグメントの 2 ページ目以降）。
...
```text
Driver 側書き込み順:
  1. side channel buffer に payload バイトを書く
  2. ring slot に (side_offset, side_len, payload_len=0) を書く
  3. ring の write_index を Release で進める

Bridge 側読み出し順:
  1. ring の write_index を Acquire で読む
  2. ring slot の side_offset / side_len を読む
  3. side channel buffer から payload バイトを読む
```
...
Release/Acquire は x86 では NOP（store-store / load-load の保証は MO
で自動）、ARM では `dmb ish` 相当（`AtomicU64::store(..., Release)` の Rust
codegen に任せる）。**実装でアーキ別 fence を書く必要はない**。
