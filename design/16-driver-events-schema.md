...
events:
  noteOn:                      # tier 省略 → inline (default)
    fields:
      channel:  { type: uint8, range: [1, 16], optional: false }
      note:     { type: uint8, range: [0, 127], optional: false }
      velocity: { type: uint8, range: [0, 127], optional: false }
  ...
  controlChange:
    fields:
      channel:    { type: uint8, range: [1, 16], optional: false }
      controller: { type: uint8, range: [0, 127], optional: false }
      value:      { type: uint8, range: [0, 127], optional: false }
...
events.yaml が宣言できるスカラー型は **driver ドメインに依存しない汎用型** のみとする。MIDI / OSC 等のプロトコル固有の値域（例: MIDI velocity 0..=127、MIDI channel 1..=16）は **`range`** で表現する（後述「driver ドメイン慣例の表現」節）。

| events.yaml 型 | msgpack 型 | デフォルト値域 |
|---|---|---|
| `int8` | msgpack `int` | `-128..=127` |
| `uint8` | msgpack `int` | `0..=255` |
| `int16` | msgpack `int` | `-32768..=32767` |
| `uint16` | msgpack `int` | `0..=65535` |
| `int32` | msgpack `int` | `i32` 範囲 |
| `uint32` | msgpack `int` | `u32` 範囲 |
| `int64` | msgpack `int` | `i64` 範囲 |
| `uint64` | msgpack `int` | `u64` 範囲 |
| `float32` | msgpack `float` | f32 |
| `float64` | msgpack `float` | f64 |
| `bool` | msgpack `bool` | `true` / `false` |
| `string` | msgpack `str` | UTF-8 文字列 |
| `bytes` | msgpack `bin` | バイト列 |

「デフォルト値域」は型自身が定める範囲。各フィールドで `range: [min, max]` を指定するとデフォルト値域の **部分集合** に絞り込める。範囲外への拡大は不可（`uint8` に `range: [-1, 300]` を指定すると events.yaml ロード時エラー）。**この規則に例外は設けない。**
...
MIDI / OSC / DMX 等の driver ドメインには「特定 bit 幅 + 特定値域」の慣例が多数ある。events.yaml schema は **これらを汎用型 + `range` の組み合わせで表現** し、ドメイン固有の専用型は導入しない（schema が driver 非依存であるため）。

代表的な MIDI ドメインの表現例:

| MIDI 概念 | wire 上の表現 | events.yaml での宣言 |
|---|---|---|
| MIDI channel（1-origin 慣例） | 0..=15（4 bit） | `{ type: uint8, range: [1, 16] }` |
| Note number / velocity / CC value | 0..=127（7 bit） | `{ type: uint8, range: [0, 127] }` |
| Pitch bend（unsigned 形式） | 0..=16383（14 bit） | `{ type: uint16, range: [0, 16383] }` |
| Pitch bend（signed 形式） | -8192..=8191（14 bit signed） | `{ type: int16, range: [-8192, 8191] }` |
...
events:
  noteOn:
    fields:
      channel:  { type: uint8, range: [1, 16] }   # 1-origin 慣例を range で表現
      note:     { type: uint8, range: [0, 127] }
      velocity: { type: uint8, range: [0, 127] }
```

**MIDI channel の 0-origin（wire 値）→ 1-origin（schema 値）の変換** は Driver 実装コード内の責務（SDK は変換を提供しない、Bridge は emit された値が `range: [1, 16]` に収まっているかを検証）。binding YAML 側も `1..=16` を使用する（`config/drivers/midi.md` 既存の慣例どおり）。

汎用型のみに揃える理由:

- schema が driver ドメインに依存しなくなり、新プロトコル追加時の特例連鎖を防げる（DMX channel 等）
- 「`range` はデフォルト値域の部分集合のみ」のルールを例外なく一律で適用できる
- 型名と値域の関係が明示的になり、driver 作者が暗黙の慣例を覚える必要がない

### 計算可能性

整数型（`int8` / `uint8` / `int16` / `uint16` / `int32` / `uint32` / `int64` / `uint64`）と浮動小数型（`float32` / `float64`）は、Layer 2 binding の `set.expr` で計算可能型として利用できる。events.yaml の `fields` に `compute: true` を明示することは要件としない。binding 側で `set.expr` 参照する変数の型が計算可能型に合致しているかを Bridge が起動時バリデーションする（events.yaml ↔ binding の整合）。
...
| 型 | filter 可 | 理由 |
|---|---|---|
| 数値（`int8` / `uint8` / `int16` / `uint16` / `int32` / `uint32` / `int64` / `uint64` / `float32` / `float64`） | ✅ | 等価比較・range 比較が安価（O(1)） |
| `bool` | ✅ | 同上 |
...
- `design/layers/01-input-driver/requirements.md` — 物理型表（bit 幅と値域の根拠資料。本書では汎用型 + `range` で表現）
