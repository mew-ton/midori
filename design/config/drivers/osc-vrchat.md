# デバイス種別定義 仕様: osc-vrchat

VRChat の OSC アバターパラメーター専用の デバイス種別定義。`osc` ドライバーを基底とし、VRChat 固有の自動正規化・アドレス制約・追加設定フィールドを宣言する。独立したドライバーではない。

デバイス種別定義 の概念と配布方法 → [`../../10-driver-plugin.md`](../../10-driver-plugin.md)
...
以下の binding は `driver: osc, device_kind: osc-vrchat` のコンテキストで記述する。
