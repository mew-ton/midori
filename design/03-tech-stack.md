...
公式ドライバープラグイン（`@midori/driver-midi`・`@midori/driver-osc`）も同じ optionalDependencies パターンで配布する。Electron アプリはこれらをデフォルト依存関係に含め、アプリインストール時に自動取得する（初回起動時に手動インストール不要）。コミュニティプラグインはユーザーが GUI から別途インストールする。
