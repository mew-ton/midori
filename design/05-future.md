...
排他的入力（UDP の排他バインド）により、同一の OSC 受信ポート（例: VRChat からの `listen_port: 9001`）を `inputs` に持てるのは同時実行中の1プロファイルだけという制約がある（→ [`config/05-profile.md`](config/05-profile.md)「同時実行プロファイル間の競合」）。複数の同時実行プロファイルが VRChat からの受信を必要とする構成を許すには、受信を1か所で受けて各ブリッジへ配る共有レシーバー（fan-out）が要る。1ドライバー↔1ブリッジの shm 1:1 不変条件と整合させる設計（レシーバーを独立プロセスにし、ブリッジごとに個別リングを張る等）が必要なため、要件が顕在化した時点で別途整理する。

### クラウドランタイム

ローカルの MIDI / OSC をブリッジ経由でクラウド側のランタイムに橋渡しする構成。アイデアレベルの記載で、初期スコープ・近期ロードマップとも無関係。現時点の設計判断が将来この形を潰さないかを点検する目的のメモとして残す。
...
構成のイメージ：ローカル側は USB MIDI / OSC を拾って WebSocket 等で流す薄いブリッジに留め、ランタイム本体はクラウドで動かす。本格化させる場合の論点（セッション概念の一級化・イベントの時刻基準・認証/テナント分離）は要件化のタイミングで別途整理する。
