# Device Config Type 仕様: osc-vrchat

VRChat の OSC アバターパラメーター専用の Device Config Type。`osc` ドライバーを基底とし、VRChat 固有の自動正規化・アドレス制約・追加設定フィールドを宣言する。独立したドライバーではない。

Device Config Type の概念と配布方法 → [`../../10-driver-plugin.md`](../../10-driver-plugin.md)
...
| デバイス構成の生成 | 手動 | アバターパラメーター JSON から自動生成 |
...
以下の binding は `driver: osc, config_type: osc-vrchat` のコンテキストで記述する。
...
| フィールド | 必須 | 説明 |
|---|---|---|
| `target` | ✅ | OSC アドレスパターン。フルパス（`/avatar/parameters/UpperExpression`）または短縮形（`UpperExpression`）で記述できる。短縮形は config type の `address_prefix` が自動付与される。`{note}` キャプチャ変数を含められる |
...
このファイルを読み込むことで、definition・binding を含むデバイス構成 YAML を自動生成できる。
...
### osc-vrchat config type の追加フィールド（additional_fields）

| フィールド | 必須 | 説明 |
|---|---|---|
| `avatar_params` | ❌ | アバターパラメーター JSON のパス。デバイス構成生成・サジェストに使用 |

このフィールドは osc-vrchat config type の `additional_fields` 宣言によってプロファイルの接続設定に追加される。
