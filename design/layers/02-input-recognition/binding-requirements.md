...
| `set` 省略時の挙動 | `adapter_kind`（省略可能） | アダプター種別定義 の `auto_normalize` 宣言がある場合、`set` 省略時の正規化ルールが追加で決まる |
...
設定仕様（set / setMap / when 記法・サンプル）→ [config/02-adapter.md#binding-セクション](../../config/02-adapter.md#binding-セクション)
...
サンプル・setMap の when 記法詳細 → [config/02-adapter.md](../../config/02-adapter.md#binding-セクション) / [config/drivers/midi.md](../../config/drivers/midi.md)
