...
アダプターの人間向け説明と AI コンテキストを保持する。Runtime は参照しない（View と AI エージェントが使用）。
...
| `spec` | ❌ | 楽器の MIDI 仕様を AI が参照できる形式でまとめた自由テキスト。AI がアダプターを生成した際に自動で書き込む |

`spec_source` を URL に限定する理由：アダプターはコミュニティで配布されるファイルであるため、ローカルパスを記録すると受け取った側で参照が壊れる。公式公開 URL であれば誰でも同一の仕様書にアクセスできる。
...
- **アダプターを AI が生成した場合**: AI が仕様書・会話内容を解析した結果を自動で書き込む
- **手動で作成した場合**: 空欄でも動作するが、AI に後から「このアダプターに合うマッパーを作って」と依頼する際の精度が向上する
...
`direction: any` のファイルは、同一プロファイル内で `inputs[].adapter` と `outputs[].adapter` の両方に同一ファイルを指定できる（双方向 MIDI 機器など）。その場合、`binding.input` と `binding.output` が同一ファイルに共存する。
...
binding:
  ...
  output:
    driver: osc
    adapter_kind: osc-vrchat   # オプション。アダプター種別定義 を適用する
    mappings:
      - ...
```

`adapter_kind` は省略可能。アダプター種別定義 プラグインがインストールされており、その `base_driver` が `driver` と一致する場合に使用できる。`adapter_kind` を指定すると、接続設定フォームへの追加フィールド・`set` 省略時の自動正規化・`address_prefix` の自動付与が有効になる。詳細 → [`../10-driver-plugin.md`](../10-driver-plugin.md)
...
| キャプチャ変数 | `set: arg1` | SysEx キャプチャバイト（events.yaml で `uint8` + `range: [0, 127]` 等で宣言される 7-bit データ）をそのまま書き込む | なし（raw 値を直接代入） |
...
1. `adapter_kind` の `auto_normalize` が宣言されている → その正規化ルールを適用
2. `adapter_kind` なし、またはルールが宣言されていない → ドライバーのデフォルト値域から target の `range` へ線形マッピング
...
**`setMap.map` の全単射バリデーション**: Bridge はアダプターのロード時に `setMap.map` を検査する。以下の条件を両方満たす場合のみ全単射と判定し、`mirror` を許可する。どちらかを満たさない場合はロードエラー。
...
入力アダプターでは Preview タブに、出力アダプターでは Monitor タブに表示される。
...
| 使用箇所 | タブ | 内容 |
|---|---|---|
| アダプター編集画面 | プレビュータブ | デバッグ用。テスト接続設定でブリッジを起動してリアルタイム確認 |
