...
変換グラフはアバター・演奏スタイルに依存するため個人ファイルとして受け渡すのが基本で、アダプターのような汎用の公開部品にはしない。ただし特定アバター向けに作られたものは、その依存物ごと配布用プロファイルパッケージに同梱して配布できる（→ [`05-profile.md`](05-profile.md)「配布用プロファイル」）。同梱されても特定アバター向けという性質は変わらない。
