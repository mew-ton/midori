...
events:
  noteOn:
    fields:
      channel:  { type: nibble, optional: false }
...
| `nibble` | msgpack `int` | `0..=15` | 4bit 整数。`range` で `[1, 16]` に上書きすると MIDI channel の表記慣例に揃えられる（後述） |
...
「デフォルト値域」は型自身が定める範囲。各フィールドで `range: [min, max]` を指定するとデフォルト値域の **部分集合** に絞り込める。範囲外への拡大は不可（`uint7` に `range: [-1, 200]` を指定すると events.yaml ロード時エラー）。
...
> ⚠️ **MIDI channel の表記**: MIDI 仕様では channel は wire 上 0..=15 で表現されるが、ユーザー視点では `1..=16` で扱うのが慣例（`config/drivers/midi.md` の binding YAML も `channel: 1..=16` を使用）。**events.yaml の宣言は `channel: { type: nibble, range: [1, 16] }` のように書く** ことで、Driver が emit する値が `1..=16` の範囲であることを Bridge が schema validator で強制する。0 オリジン → 1 オリジンの変換は **Driver の実装コード内で行う責務**（SDK は変換を提供しない、Bridge は変換結果のみ検証）。

> 設計判断: `range: [1, 16]` は `nibble` のデフォルト値域 `0..=15` の部分集合ではないが、**MIDI channel は意味上 1-origin で扱うべき例外** として、`nibble` 型に限り `[1, 16]` への range 上書きを許容する特例ルールとする（events.yaml ローダーが `nibble + range:[1,16]` の組合せを特別扱いする）。
...
events:
  noteOn:
    fields:
      channel:  { type: nibble, range: [1, 16] }
      note:     { type: uint7 }
      velocity: { type: uint7 }
```
...
| `range` のデフォルト値域逸脱 | `range` が型のデフォルト値域の部分集合になっていない（`nibble + range:[1,16]` の MIDI channel 特例を除く） |
