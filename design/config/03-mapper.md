...
変換グラフはアバター・演奏スタイルに依存するため個人ファイルとして受け渡すのが基本で、アダプターのような汎用の公開部品にはしない。ただし特定アバター向けに作られたものは、ワークスペース内の依存アダプター・変換グラフごと配布用プロファイルパッケージに同梱して配布できる（プラグイン由来のアダプターは同梱せずマニフェストに依存プラグインとして記録し、ドライバー本体も含まない → [`05-profile.md`](05-profile.md)「配布用プロファイル」）。同梱されても特定アバター向けという性質は変わらない。

マッパーファイルは純粋なグラフ定義のみを持つ。どのアダプターを使うかはプロファイル（`profiles/*.yaml`）が宣言し、ランタイムがプロファイルをもとにアダプターを解決する。マッパーのポート記法で使う `<device_id>`（例: `input.yamaha-els03.`）はプロファイルの `inputs[].id` / `outputs[].id` に対応する。
