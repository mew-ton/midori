...
- `devices/` にデバイス構成が1つもなく、`drivers` / `device_kinds` / `render_components` も宣言されていない → 警告（インストールは可能）
