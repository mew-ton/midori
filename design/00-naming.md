...
| **プロファイル** | 入力アダプター・変換グラフ・出力アダプター・実デバイス接続設定を束ねた実行単位。ブリッジはプロファイルを元に動作する。複数のプロファイルを同時に実行できる |
| **ローカルプロファイル** | 実機デバイス名・送信先（`connection`）を直接宣言する、利用者環境固有の非公開プロファイル |
| **配布用プロファイル** | 環境依存の接続設定（`connection`）を持たないプロファイル。配布を前提とし、利用者の環境で実機デバイス・送信先を割り当てて（アクティベート）使う。エレクトーンアセット等に同梱される |
| **プロファイルパッケージ** | 配布用プロファイルと、それが依存するアダプター・変換グラフを1単位にまとめた配布物。アプリに取り込むとワークスペースへ統合される。ドライバー本体は含まず、依存ドライバーはプラグインとして別途インストールする |
| **アクティベート** | 取り込んだ配布用プロファイルに、利用者環境の実機デバイス・送信先（`connection`）を割り当てて実行可能にする操作 |
| **エクスポート（公開用に出力）** | ローカルプロファイルから接続設定を除き、依存物を集めてプロファイルパッケージを書き出す作者側の操作 |
| **取り込み（インポート）** | プロファイルパッケージをアプリで読み込み、現在のワークスペースへ統合する利用者側の操作。エクスポートの逆 |
| **出力バッティング** | 同時実行する複数プロファイルが、出力アダプターで解決した同じ送信先（同じ OSC アドレス＝同じアバターパラメーター等）へ書き込む競合。検出はアプリが行い、起動不可（`block`）か警告のみ（`warn`）かは出力ドライバーの `output_conflict` 宣言が決める |
...
| **プロファイルパッケージマニフェスト** | `.midori/package.yaml` | プロファイルパッケージのマニフェスト。パッケージ名・バージョン・収録する配布用プロファイル・同梱する依存アダプター・変換グラフ・依存プラグイン・依存ドライバーを宣言する |
