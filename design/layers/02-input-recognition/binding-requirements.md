...
| フィールド | 型を確定するもの | 意味 |
|---|---|---|
| `from` | `binding.driver` | `driver: midi` と宣言した時点で `from` の有効フィールド（`channel` / `type` / `controller` 等）が確定する |
...
| `button` | `<component_id>.<value_name>` | `upper_sustain.pressed` |
