...
マッパーファイル（`transform` で指定）はグラフ定義のみを持ち、どのデバイス構成を使うかは持たない。デバイス構成ファイルの解決はプロファイルが担い、`inputs[].id` / `outputs[].id` がマッパーのポート記法（`input.<id>.` / `output.<id>.`）と対応する。
...
outputs:
  - id: vrchat-default                             # 省略時はデバイスファイルのベース名から自動生成
    ...
    connection:
      ...
      host: 127.0.0.1
      port: 9000
      avatar_params: "C:/Users/.../OSC/.../Avatars/avtr_xxx.json"  # 任意（osc-vrchat デバイス種別定義の追加フィールド）
```
...
デバイス種別定義（例: `osc-vrchat`）が `additional_fields` を宣言している場合、それらのフィールドも接続設定に追加される。`avatar_params` 等は osc-vrchat デバイス種別定義が宣言した追加フィールドであり、このテーブルには含まない。詳細 → [`../10-driver-plugin.md`](../10-driver-plugin.md)
